...
ModbusMaster node(1); //Indicamos la ID del maestro, en este caso utilizamos el 1


void setup()
{
  // initialize Modbus communication baud rate
  node.begin(9600);  //iniciamos la comunicacion indicando la velocidad en baudios 9600
  pinMode(12,OUTPUT);
  pinMode(13,OUTPUT);
}
int a;
 ...
 if (result == node.ku8MBSuccess)
  {
      ...
      data[1] = node.getResponseBuffer(1);     // metenmos en data[1] el dato que lee del esclavo del bite 1
  
  
      if (data[0]%2==0)  //si es un numero par actvivamos salida 12 y si no la desactivamos
       {
